    ...
    <div class="h-[1097px] relative bg-stone-50">
      ...
      <div class="h-[528px] px-10 py-[38px] left-[240px] top-[144px] absolute bg-white rounded-2xl flex-col justify-end items-start gap-10 inline-flex">
        <div class="self-stretch justify-start items-start gap-[623px] inline-flex">
          <div class="text-black text-2xl font-bold font-poppins leading-9 tracking-tight">Edit Pengguna</div>
        </div>
            ...
            <div class="self-stretch justify-start items-start gap-10 inline-flex">
              <div class="w-[500px] flex-col justify-start items-start gap-1 inline-flex">
                <div class="self-stretch h-20 flex-col justify-start items-start gap-2 flex">
                  ...
                  <input v-model="name" type="text" id="name" name="name" placeholder="Input nama" class="self-stretch px-4 py-3 rounded-lg border border-stone-200 justify-between items-center inline-flex">
                </div>
              </div>
        ...
        <div class="justify-start items-end gap-6 inline-flex">
          <button @click="editUser" class="px-6 py-3 bg-red-500 rounded-lg shadow justify-center items-center gap-2 flex">
            <div class="text-white text-base font-semibold font-poppins leading-normal tracking-tight">Simpan Perubahan</div>
          </button>
  ...
  export default {
    data() {
      return {
        userId: '',
        selectedRole: '',
        roles: ["TECHNICIAN", "OPERATION", "SELLER", "CUSTOMER_CARE", "SCHEDULLER", "LEAD_SELLER", "PURCHASER"],
    ...
    methods: {
      async getUser(userId) {
        try {
          const response = await fetch(`https://regist-api-test.transtrack.id/api/v1/users/?filter_columns=id&filter_keys=${userId}`, {
            method: 'GET',
            headers: {
      ...
      async editUser() {
        try {
          const response = await fetch(`https://regist-api-test.transtrack.id/api/v1/users/${this.userId}`, {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': 'Bearer 1389|z0cI5RADaaVLIauLbaIs2treHaq4UDtoNC89xsI5',
          ...
          if (response.ok) {
            ...
            alert('Failed to edit user:', response.statusText);
          }
        } catch (error) {
          console.error('An error occurred:', error);
        }
